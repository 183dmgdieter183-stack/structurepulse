...
class ModuleStats:
    ...
    is_boilerplate: bool = False   # propagated from ParseResult
    ...
    def build(self, parse_results: list) -> GraphResult:
        ...
        for pr in parse_results:
            ...
            ms = ModuleStats(
                ...
                is_boilerplate=getattr(pr, "is_boilerplate", False),
        ...
        # Stem-to-full-path lookup so tests/agents can pass bare module names
        from pathlib import Path as _Path
        _stem_to_path: dict = {}
        for mf in module_files:
            _stem_to_path[_Path(mf).stem] = mf

        def _resolve_dst(dst: str) -> str:
            if dst in module_files:
                return dst
            stem = dst.split(".")[-1] if "." in dst else dst
            if stem in _stem_to_path:
                return _stem_to_path[stem]
            return dst
        ...
        for pr in parse_results:
            for e in pr.edges:
                if e["kind"] == "import":
                    dep_graph[pr.file].add(_resolve_dst(e["dst"]))
        ...
        if n > 1:
            ...
            # Use the already-resolved dependency_graph to count cross-edges
            # dep_graph values are already resolved to full module paths
            cross_edges = set()
            for src_mod, dsts in result.dependency_graph.items():
                for dst in dsts:
                    if dst in known_modules and dst != src_mod:
                        cross_edges.add((src_mod, dst))
