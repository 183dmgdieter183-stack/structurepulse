...
def _evaluate_fail_conditions(conditions: list, metrics, graph) -> list[str]:
    """
    Parse and evaluate --fail-on conditions against scan results.

    Supported condition syntax:
      cycles                 — any cycle present
      cycles>N               — more than N cycles
      inconsistencies        — any heuristic warning present
      inconsistencies>N      — more than N warnings
      shi<N                  — SHI below threshold N  (e.g. shi<0.7)
      shi<=N                 — SHI at or below N
      coupling>N             — coupling density above N
      fd<N / fd>N            — fractal dimension outside range
    ...
    for raw in conditions:
        ...
        if cond == "cycles":
            if graph.cycle_count > 0:
                violations.append(
                    f"cycles: found {graph.cycle_count} dependency cycle(s) (threshold: 0)"
                )
            continue

        # ── inconsistencies ────────────────────────────────────────────────
        if cond == "inconsistencies":
            n = len(metrics.inconsistencies)
            if n > 0:
                violations.append(
                    f"inconsistencies: found {n} role-inconsistency warning(s)"
                )
            continue

        # ── parametric conditions: metric op value ─────────────────────────
        m = re.fullmatch(r"(cycles|inconsistencies|shi|coupling|fd)([<>]=?)([0-9.]+)", cond)
        if not m:
            violations.append(f"unknown --fail-on condition: '{raw}' (ignored)")
            continue

        metric_name, op, raw_val = m.group(1), m.group(2), m.group(3)
        try:
            threshold = float(raw_val)
        except ValueError:
            violations.append(f"invalid threshold in '{raw}'")
            continue

        actual = {
            "shi":             metrics.shi,
            "coupling":        metrics.coupling_density,
            "fd":              metrics.fd,
            "cycles":          float(graph.cycle_count),
            "inconsistencies": float(len(metrics.inconsistencies)),
        }[metric_name]

        def _check(a, o, t):
            return {"<": a < t, "<=": a <= t, ">": a > t, ">=": a >= t, "==": a == t}.get(o, False)

        if _check(actual, op, threshold):
            violations.append(
                f"{metric_name}{op}{threshold}: actual={actual}"
            )
...
def cmd_scan(args):
    ...
    _extra_excludes = getattr(args, "exclude", None) or []

    if not json_out:
        print(BOLD(CYAN(f"\n  StructurePulse — scanning {path}")))
        if _extra_excludes:
            print(DIM(f"  excluding: {chr(32).join(_extra_excludes)}"))
        print(DIM("  " + "─" * 52))

    # 1. parse
    parse_results = parse_path(path, extra_patterns=_extra_excludes or None)
    ...
    if json_out:
        # Evaluate fail-on conditions and embed in JSON output
        fail_conditions = getattr(args, "fail_on", None) or []
        violations = _evaluate_fail_conditions(fail_conditions, metrics, graph)
        output["ci"] = {
    ...
    if graph.dependency_cycles:
        ...
        for cycle in graph.dependency_cycles[:5]:
            parts = []
            for i, module_path in enumerate(cycle):
                name = Path(module_path).name
                next_path = cycle[(i + 1) % len(cycle)]
                line = graph.import_lines.get((module_path, next_path), 0)
                if line:
                    parts.append(f"{name}:{line}")
                else:
                    parts.append(name)
            print(f"  → {' → '.join(parts)} → ...")
        if len(graph.dependency_cycles) > 5:
            print(f"  {DIM(f'... and {len(graph.dependency_cycles)-5} more cycles')}")
        print()
    ...
    # ── Role-Inconsistency warnings ───────────────────────────────────────
    if metrics.inconsistencies:
        print(f"  {BOLD(GOLD('[!] ROLE INCONSISTENCY'))}")
        print(DIM("  " + "─" * 52))
        for w in metrics.inconsistencies:
            icon = RED("⚠") if w.severity == "warning" else GOLD("→")
            print(f"  {icon}  {BOLD(w.kind):<30}  {Path(w.module_path).name}")
            print(f"     {w.message}")
            print(f"     {DIM(w.detail)}")
        print()

    # ── --explain output ──────────────────────────────────────────────────
    if getattr(args, "explain", False):
        print(f"  {BOLD('ROOT CAUSE ANALYSIS')}")
        for line in metrics.explanation:
            if line.startswith("  "):
                print(f"  {DIM(line.strip())}")
            else:
                icon = "✓" if "✓" in line else "⚠" if any(w in line for w in ("rigid","chaos","critical","warning","cycle","Coupling")) else "→"
                color = GREEN if "✓" in line else (GOLD if "warning" in line.lower() or "moderate" in line.lower() else RED)
                print(f"  {color(icon)}  {line}")
        bp_count = sum(1 for pr in parse_results if getattr(pr, "is_boilerplate", False))
        if bp_count:
            print(f"  {DIM(f'  ({bp_count} boilerplate file(s) excluded from FD calculation)')}")
        print()
    ...
    violations = _evaluate_fail_conditions(fail_conditions, metrics, graph)
    if violations:
        print(f"  {RED(BOLD('✗ CI/CD GATE FAILED'))}")
        print(DIM("  " + "─" * 52))
        for v in violations:
            print(f"  {RED('✗')}  {v}")
        print()
...
def main():
    ...
    p_scan.add_argument("--explain", action="store_true", help="Root cause analysis of metric values")
    p_scan.add_argument("--exclude", nargs="+", metavar="PATTERN", default=None,
                        help="Extra exclude patterns, e.g. --exclude tests/ 'test_*.py' migrations/")
    p_scan.add_argument("--fail-on", nargs="+", metavar="CONDITION", dest="fail_on", default=None,
                        help=("Exit 1 if condition met. Supported: cycles, inconsistencies, "
                              "shi<N (e.g. shi<0.7), shi<=N, coupling>N, cycles>N"))

    p_watch = sub.add_parser("watch", help="Live-reload scan — re-scans on every file save")
    p_watch.add_argument("path", nargs="?", default=".", help="Repository path")
    p_watch.add_argument("--interval", type=float, default=1.5, help="Poll interval in seconds (default: 1.5)")
    p_watch.add_argument("--min-shi", type=float, default=0.8, dest="min_shi", help="SHI alert threshold (default: 0.8)")
    p_watch.add_argument("--explain", action="store_true", help="Show root cause analysis on each scan")
    p_watch.add_argument("--no-clear", action="store_true", dest="no_clear", help="Disable terminal overwrite between scans")

    p_lsp = sub.add_parser("lsp", help="Start Language Server Protocol server (JSON-RPC 2.0 over stdio)")
    p_lsp.add_argument("--workspace", type=str, default=".", dest="workspace", help="Workspace root path (default: .)")
    p_lsp.add_argument("--min-shi",   type=float, default=0.65, dest="min_shi", help="SHI threshold for diagnostics (default: 0.65)")
    p_lsp.add_argument("--log-file",  type=str, default=None, dest="log_file", help="Write LSP debug log to file")
    ...
    if args.command == "scan":
        ...
        from structurepulse.watch import run_watch
        import os as _os
        run_watch(
            root=_os.path.abspath(args.path or "."),
        ...
        from structurepulse.lsp.server import StructurePulseLSP, _setup_logging
        _setup_logging(getattr(args, "log_file", None))
        import os as _os
        server = StructurePulseLSP(
            workspace_root=_os.path.abspath(getattr(args, "workspace", ".")),
            min_shi=getattr(args, "min_shi", 0.65),
        )
        server.serve()
